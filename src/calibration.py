...
class PlattCalibrator:
    ...
    def fit(self, p, y):
        ...
        # Logistic regression on logits is often more stable
        logits = np.log(x / (1 - x))
    ...
    def predict(self, p):
        x = np.asarray(clip_probs(p)).reshape(-1, 1)
        logits = np.log(x / (1 - x))
        out = self.model.predict_proba(logits)[:, 1]
...
def calibration_table(y_true, p_pred, bins: int = 10) -> pd.DataFrame:
    ...
    for b in range(bins):
        ...
        n = int(mask.sum())
        if n == 0:
            rows.append({"bin": b, "bin_left": edges[b], "bin_right": edges[b + 1], "n": 0, "avg_pred": np.nan, "emp_rate": np.nan, "abs_gap": np.nan})
            continue
        avg_pred = float(p_pred[mask].mean())
        emp_rate = float(y_true[mask].mean())
        abs_gap = abs(avg_pred - emp_rate)
        rows.append({"bin": b, "bin_left": edges[b], "bin_right": edges[b + 1], "n": n, "avg_pred": avg_pred, "emp_rate": emp_rate, "abs_gap": abs_gap})
